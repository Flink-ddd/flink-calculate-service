...
import com.panda.flink.service.FlinkOrderService;
import com.panda.flink.sink.JdbcWriter;
import com.panda.flink.sink.RedisDataRichSink;
import com.panda.flink.source.KafkaOrderSourceFunction;
...
/**
 * @author muxiaohui
 * @Description Flink数据流实时计算示例：启动对接kafka数据，流处理flink业务服务job，模拟对用户消费订单信息多维度聚合统计;
 */
@Slf4j
public class StartFlinkKafkaOrderServer {
    static Logger logger = LoggerFactory.getLogger(StartFlinkKafkaOrderServer.class);
    /**
     * 窗口事件时间
     */
    ...
    public static void main(String[] args) {
        System.out.println("out:开始启动StartFlinkKafkaOrderServer服务");
        logger.info("开始启动StartFlinkKafkaOrderServer服务");
        //无界数据流
        ...
        //读取kafka数据源
        KafkaOrderSourceFunction kafkaSourceFunction = new KafkaOrderSourceFunction();
        DataStream<Order> source = kafkaSourceFunction.run(env);
        //下单数据写入MySQL
//        sendOrderDataToMysql(source);
        //统计订单总量和总额、最新订单窗口总量
        ...
        //执行JOB
        try {
            env.execute("订单聚合统计JOB");
        } catch (Exception e) {
            logger.error("订单聚合统计JOB,执行异常！", e);
        }
    }

//    private static void sendOrderDataToMysql(DataStream<Order> source) {
//        source.addSink(new JdbcWriter());
//        log.info("读取flink算子中的下单数据写入MySQL成功");
//    }
    ...
    private static void countOrderNumAndAmount(DataStream<Order> orderDataStream) {
        DataStream<Tuple3<String, Integer, BigDecimal>> output =
                FlinkOrderService.commonCount("countOrderNumAndAmount", QuotaEnum.DEFAULT, orderDataStream, RedisCommand.SET, "FLINK_ORDER_TOTAL_NUM", "FLINK_ORDER_TOTAL_PRICE", true);
    ...
    private static void countGenderShoppingNumAndAmount(DataStream<Order> orderDataStream) {
        FlinkOrderService.commonCount("countGenderShoppingNumAndAmount", QuotaEnum.GENDER, orderDataStream, RedisCommand.HSET, "FLINK_ORDER_GENDER_TOTAL_NUM", "FLINK_ORDER_GENDER_TOTAL_PRICE", true);
    }
    ...
    private static void countGoodsTypeNumAndAmount(DataStream<Order> orderDataStream) {
        FlinkOrderService.commonCount("countGoodsTypeNumAndAmount", QuotaEnum.GOODS_TYPE, orderDataStream, RedisCommand.HSET, "FLINK_ORDER_GOODS_TYPE_TOTAL_NUM", "FLINK_ORDER_GOODS_TYPE_TOTAL_PRICE", true);
    }
    ...
    private static void countBrandNumAndAmount(DataStream<Order> orderDataStream) {
        FlinkOrderService.commonCount("countBrandNumAndAmount", QuotaEnum.BRAND, orderDataStream, RedisCommand.HSET, "FLINK_ORDER_BRAND_TOTAL_NUM", "FLINK_ORDER_BRAND_TOTAL_PRICE", true);
    }
    ...
    private static void countUserAmount(DataStream<Order> orderDataStream) {
        ...
        FlinkOrderService.commonCount("countUserAmount", QuotaEnum.USER, orderDataStream, RedisCommand.HSET, null, "FLINK_ORDER_USER_RANKING", true);
    }

    /**
     * 按性别统计每1分钟下单总量,并记录下单时间(单独流处理)
     *
     * @param orderDataStream
     */
